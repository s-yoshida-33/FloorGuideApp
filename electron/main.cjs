...
// Default Genre Memo Settings
const DEFAULT_GENRE_MEMO_SETTINGS = {
  maxDisplayItems: 3,
  excludedKeywords: [],
};
...
const INITIAL_GENRE_MEMO_SETTINGS = bundledDefaults.genreMemoSettings || DEFAULT_GENRE_MEMO_SETTINGS;
...
function loadSettings() {
  const base = {
    ...
    genreMemoSettings: INITIAL_GENRE_MEMO_SETTINGS,
    imageSettings: {
      floorMaps: {
        '1F': '',
        '2F': '',
  ...
  try {
    ...
    const merged = {
      ...
      genreMemoSettings: (() => {
        const stored = parsed.genreMemoSettings || {};
        const baseSettings = base.genreMemoSettings || DEFAULT_GENRE_MEMO_SETTINGS;
        
        return {
            maxDisplayItems: typeof stored.maxDisplayItems === 'number' ? stored.maxDisplayItems : baseSettings.maxDisplayItems,
            excludedKeywords: Array.isArray(stored.excludedKeywords) ? stored.excludedKeywords : baseSettings.excludedKeywords
        };
      })(),
      locationIcons: {
...
/**
 * Broadcast genre memo settings changes to renderer processes
 */
function broadcastGenreMemoSettings(genreMemoSettings) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('genre-memo-settings-updated', genreMemoSettings);
  }
}
...
function createMainWindow() {
  ...
  mainWindow.webContents.on('did-finish-load', () => {
    ...
    broadcastGenreMemoSettings(settings.genreMemoSettings);
...
ipcMain.handle('get-genre-memo-settings', () => {
  const settings = loadSettings();
  logger.debug('IPC get-genre-memo-settings');
  return settings.genreMemoSettings || INITIAL_GENRE_MEMO_SETTINGS;
});

ipcMain.handle('save-genre-memo-settings', (_event, genreMemoSettings) => {
  logger.info('IPC save-genre-memo-settings');
  const settings = saveSettings({ genreMemoSettings });
  broadcastGenreMemoSettings(settings.genreMemoSettings);
  return settings.genreMemoSettings;
});
